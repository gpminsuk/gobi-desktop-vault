...
1. **Value-First**: 3분 안에 첫 AHA 모먼트 (클리핑+글)
2. **Agent-does-it**: 묻지 말고 해주기 (비파괴적 행동은 기본 실행)
3. **Voice-First**: 모든 단계 음성 진행
4. **적응형**: 사용자 환경 맞춤
5. **결정 포인트 최소화**: DRB는 기본 ON, 목표 선택 제거
6. **시각적 피드백**: 진행률 & 결과
...
- **약속한 동작은 반드시 실행**: "동기화할게요", "만들어드릴게요" 등 약속한 작업은 말한 직후 반드시 실행한다. 다음 단계로 넘어가기 전에 실행을 확인한다.
- **현재 파일 참조**: 사용자가 "현재 작업중인 문서", "이 파일" 등으로 현재 문서를 언급하면 대화 컨텍스트에서 최근 다룬 파일을 우선 추론하고, 모호하면 경로를 짧게 되묻는다.
...
- 에이전트: "프로필이 만들어졌어요! 볼트 루트의 `BRAIN.md`에서 확인할 수 있어요."
...
### BRAIN.md + Context/interest.md 즉시 생성

BBG 완료 후, **확인 없이** 두 파일을 즉시 생성/업데이트한다:
...
**`Context/interest.md`**:
- BBG/BBF에서 확보된 관심 분야를 "주요 관심 분야" 섹션에 bullet으로 추가
- 명시적으로 "지금 배우는 중"이라고 언급된 항목은 "현재 학습 중" 섹션에
- 업데이트 로그 표에 한 줄 추가: `YYYY-MM-DD | 온보딩에서 N개 관심사 추가 | BBG/BBF`
- 파일이 비어있는 템플릿 상태면 "_아직 없음_" 자리표시자를 실제 항목으로 교체
...
**1단계: 관심사 추출**
- `Context/interest.md`의 "주요 관심 분야" + BRAIN.md/BRAIN_PROFILE.md에서 관심사/전문 분야 추출
...
**3단계: 시드 글 즉시 생성 (2-3개)**
- 클리핑 내용과 프로필을 분석하여 `Articles/` 폴더에 시드 글 생성
- 각 글 파일에 frontmatter(`title`, `created`, `tags`) 포함
- 관련 클리핑에 대한 위키링크 연결

> 클리핑을 바탕으로 짧은 글도 만들어뒀어요. 주제별로 자료가 연결돼 있어요.
...
- 에이전트: 해당 소스에 맞는 연결 방법 안내 (Google Workspace, Notion Importer 등)

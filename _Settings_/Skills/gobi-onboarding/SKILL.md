...
##### I. 에이전트 주도 콘텐츠 수집
- 사용자: "네가 알아서 가져와" / "내가 좋아할만한 걸로 골라줘" / "추천해줘"
- 에이전트: "프로필을 보고 맞춤 자료를 찾아올게요!"
- 프로필(BRAIN.md, BRAIN_PROFILE.md)에서 관심사/전문 분야 추출
- 관심사 기반 웹 검색 수행 (WebSearch + WebFetch)
- 검색 결과를 `Ingest/Clippings/`에 마크다운 클리핑으로 저장
  - 파일명: `YYYY-MM-DD {제목}.md`
  - frontmatter에 `source`, `author`, `tags: [clipping]` 포함
- 에이전트: "프로필을 바탕으로 [주제] 관련 자료를 가져왔어요! 옵시디언에서 확인해보세요."
- (`obsidian_enabled = true`인 경우 `obsidian open`으로 클리핑 파일을 열어준다)

### 3-2. Timeline & Graph 시각화

인제스트 완료 후, 결과를 시각적으로 보여준다.

**옵시디언 활성화 시 클리핑/토픽 추가 후 동작** (`obsidian_enabled = true`):
- 클리핑 파일이 저장되면 `obsidian open`으로 해당 파일을 즉시 열어준다.
- `obsidian command id=graph:open`으로 그래프 뷰를 열어 새 연결을 시각적으로 보여준다.
- 토픽이 추가/연결되면 그래프 뷰를 다시 갱신하여 변화를 보여준다.
...
**시드 토픽 즉시 생성**: TIU 크론이 다음 실행까지 대기하면 사용자가 빈 그래프를 보게 된다. TIU 활성화 직후, 프로필(BRAIN.md/BRAIN_PROFILE.md)과 인제스트된 자료를 분석하여 2-3개 시드 토픽을 `Topics/` 폴더에 즉시 생성한다.
- 각 토픽 파일에 frontmatter(`title`, `created`, `tags`) 포함
- 관련 클리핑/문서에 대한 위키링크 연결
- 에이전트: "토픽 인덱스를 켰어요. 바로 확인할 수 있도록 시드 토픽도 만들어뒀어요!"
- (`obsidian_enabled = true`인 경우 `obsidian command id=graph:open`으로 그래프 뷰를 열어 토픽 연결을 시각적으로 보여준다)
...
![[_Settings_/Skills/gobi-onboarding/Gobi Onboarding Flow.canvas]]
...
`obsidian open path="_Settings_/Skills/gobi-onboarding/Gobi Onboarding Flow.canvas"` 실행하여 캔버스를 직접 연다.
...
| `Gobi Onboarding Flow.canvas` | 3-5 온보딩 캔버스 | 온보딩 상황 캔버스 (Ingest → Summary → Topics → Publish 흐름) | 옵시디언 활성화 시만 |

...
> 네 단계로 진행할 건데, 십 분이면 충분해요.
> 프로필 만들고, 지식을 체험하고, 콘텐츠를 만들어보고, 마무리하는 순서예요.
> 바로 시작할까요?
...
| 키워드 | 기능 |
|--------|------|
| "지식 공유" | 스페이스 선택 → 참여 + 브레인 업데이트 + Welcome 인사 |
...
사용자가 "지식 공유" 키워드를 말하면, **반드시 스페이스를 먼저 선택**한 뒤 진행한다. 자동으로 특정 스페이스에 포스팅하지 않는다.

1. `gobi space list`로 가용 스페이스 목록 조회
2. 사용자에게 스페이스 선택 요청: "어떤 스페이스에서 시작할까요?"
3. 사용자가 선택한 스페이스에서 브레인 업데이트 + Welcome 스레드 생성

...
**전제 조건**: 옵시디언에서 볼트가 열려 있어야 CLI 명령이 작동한다. 2단계 옵시디언 연동에서 사용자가 볼트를 직접 열도록 안내하고, `obsidian version`으로 CLI 연결을 확인한 뒤에만 아래 규칙을 적용한다. 볼트가 열려 있지 않으면 `obsidian open` 등을 시도하지 않는다.
...
> 전체 온보딩 흐름을 캔버스로 열어드렸어요. 지금 우리는 두 번째 단계에 있어요.
...
- 사용자 동의시: `.gobi/syncfiles`에 아래 파일 추가
  ...
  - **사용자가 동의하면 즉시 실행한다. "좋아요!" 후 다음 질문으로 넘어가지 않는다.** syncfiles 명령 실행 후 결과를 확인하고 사용자에게 완료를 알린 뒤 다음 단계로 진행한다.
...
> 프로필을 바탕으로 맞춤 자료를 가져올게요!
> 관심사에 맞는 아티클을 찾아서 클리핑하고, 주제별로 정리까지 해드릴게요.

#### 에이전트 자동 실행 흐름

사용자에게 "어떤 자료를 가져올까요?"라고 묻지 않는다. 에이전트가 주도적으로 아래 흐름을 실행한다.
...
클리핑 생성이 완료된 후, "앞으로 직접 할 수 있는 도구"로 웹 클리퍼를 소개한다.

> 방금은 제가 자동으로 가져왔는데, 앞으로 직접 클리핑하고 싶으면 웹 클리퍼가 편해요. 브라우저 확장 프로그램인데, 웹페이지를 바로 볼트에 저장할 수 있어요.

**옵시디언 활성화 시** (`obsidian_enabled = true`인 경우):
...
> 나중에 관심 있는 글을 발견하면 웹 클리퍼로 한 번에 저장할 수 있어요.

**옵시디언 미활성화 시** (`obsidian_enabled = false`): Web Clipper는 Post-boarding에서 안내

#### 사용자 응답 시나리오 (후속 대응)

에이전트 주도 클리핑 완료 후, 사용자가 추가 질문이나 요청을 하면 대응한다.
...
그래프 뷰는 3-1에서 이미 열어주었으므로, 여기서는 타임라인을 소개한다.

> 타임라인으로도 볼 수 있는데, 시간 순서대로 내가 어떤 자료를 모았는지 한눈에 볼 수 있어요.
> 자료를 더 넣으면 타임라인도 자동으로 업데이트돼요.
...
### 3-4. Topic Index 자동 업데이트 활성화

3-1에서 시드 토픽을 이미 생성했으므로, 여기서는 TIU(자동 업데이트) 활성화만 진행한다.

#### 활성화 전 확인 (필수)
1. `orchestrator.yaml`의 `nodes` 배열에서 `Topic Index Update (TIU)` 항목 존재 여부 확인
2. 이미 존재하면: "토픽 자동 업데이트는 이미 켜져 있어요!" → 다음 단계로 진행
3. 없으면: 사용자 동의 후 `orchestrator.yaml`에 TIU 노드 추가

> 아까 만든 토픽들이 앞으로도 자동으로 업데이트되도록 설정할게요.
> 새 자료가 들어오면 관련 토픽에 자동으로 연결돼요.

#### 사용자 응답 시나리오

##### A. 활성화
- 사용자: "네, 켜주세요" / "좋아요"
- 에이전트: "토픽 자동 업데이트를 켰어요. 앞으로 새 자료가 들어오면 토픽이 자동으로 갱신돼요."
...
##### C. 혼란
- 사용자: "인덱스가 뭐예요?" / "왜 필요해요?"
- 에이전트: "인덱스는 도서관의 목록이라고 생각하시면 돼요. 자료를 그냥 쌓아두면 찾기 어렵잖아요. 인덱스가 있으면 '이 주제 자료 보여줘' 하면 바로 찾아줘요. 아까 만든 토픽들이 바로 그 인덱스예요."
...
> 2단계에서 열어드린 온보딩 캔버스를 다시 확인해보세요. 인제스트, 서머리, 토픽, 퍼블리시까지 — 이 흐름이 자동으로 돌아가요.

### 3-6. 음성 캔버스 (Capture Tab)

> 자료를 가져오는 것 외에, 내 생각도 지식이에요.
> Capture 탭에서 녹음을 시작하면 말하는 내용이 자동으로 캔버스에 정리돼요.
> 브레인스토밍이나 아이디어 정리할 때 유용해요. 한번 체험해볼까요?

#### 사용자 응답 시나리오

##### A. 체험
- 사용자: "네, 해볼게요"
- 에이전트: "좋아요! Capture 탭에서 녹음을 시작해보세요. 말씀하시는 내용이 자동으로 캔버스로 만들어져요."
- (사용자가 Capture 탭에서 녹음 → ACB가 자동으로 캔버스 생성)
- 에이전트: "캔버스가 만들어졌어요! 옵시디언에서 확인해보세요. 주제별로 자동 분류된 걸 볼 수 있어요."

##### B. 건너뛰기
- 사용자: "나중에요" / "안 할래요"
- 에이전트: "알겠어요. Capture 탭에서 녹음하면 언제든 캔버스가 자동 생성돼요."

##### C. 질문
- 사용자: "캔버스가 뭐예요?"
- 에이전트: "캔버스는 생각을 시각적으로 정리하는 도구예요. 마인드맵처럼 주제별로 카드가 배치되고 연결돼요. 녹음하면 AI가 자동으로 이걸 만들어줘요."
...
## 5단계: 지식 공유 (Share) — 선택 단계

**이 단계는 선택(optional)이다.** 4단계(콘텐츠 생성) 완료 후 에이전트가 제안하고, 사용자가 건너뛸 수 있다.

### 에이전트 오프닝

> 다섯 번째 단계는 지식 공유예요. 이건 선택 단계라서 건너뛰어도 괜찮아요.
> 지식 공유도 체험해볼까요, 아니면 바로 마무리할까요?

#### 사용자 응답 시나리오

##### A. 체험
- 사용자: "해볼게요" / "네"
- 에이전트: "좋아요! 커뮤니티 스페이스에 참여하고, 첫 브레인 업데이트를 게시하고, Welcome 쓰레드에 인사도 남겨볼 거예요."

##### B. 건너뛰기
- 사용자: "건너뛸게요" / "마무리할게요"
- 에이전트: "알겠어요! 나중에 언제든 '지식 공유'라고 말하면 도와드릴게요. 바로 마무리할게요."
- (온보딩 완료 Closing으로 전환)
...
1. BRAIN.md / BRAIN_PROFILE.md에서 interests/skills 추출
2. 5-10개 검색 쿼리 생성
3. 각 쿼리로 `gobi brain search --query` 실행 (병렬)
4. 관련성 순 정렬, 중복 제거
5. 상위 3-5개에 대해 `gobi brain ask --vault-slug <slug> --question "What are your main areas of expertise?"` 실행
6. 매칭 결과를 대화형으로 제시

> [검색 결과 요약]. 관심 있는 브레인이 있으면 질문을 보내볼 수 있어요.

#### 사용자 응답 시나리오

##### A. 관심 표현
- 사용자: "오, 이 브레인 궁금해요" / "[브레인 이름]에 질문해볼래요"
- 에이전트: `gobi brain ask --vault-slug <slug> --question "[사용자 질문]"` 실행
- 에이전트: "답변이 왔어요! [답변 요약]. 더 궁금한 게 있으세요?"

##### B. 건너뛰기
- 사용자: "나중에요" / "넘어갈게요"
- 에이전트: "알겠어요. 나중에 '브레인 탐색'이라고 말하면 언제든 찾아볼 수 있어요."

##### C. 결과 없음
- 검색 결과가 없거나 부족한 경우
- 에이전트: "아직 비슷한 관심사의 브레인이 많지 않아요. 커뮤니티가 커지면 더 많은 브레인을 만날 수 있을 거예요."

### 5-1. 스페이스 참여 (에이전트 직접 처리)

> 커뮤니티 스페이스에 참여하면 다른 전문가들과 지식을 나눌 수 있어요.
> 스페이스는 주제별 커뮤니티 공간이라고 생각하시면 돼요.
> 어떤 스페이스가 있는지 먼저 보여드릴게요.

**에이전트가 gobi-cli로 직접 처리한다** (`gobi space warp`는 인터랙티브 명령어이므로 사용하지 않는다):

1. `gobi --json space list` 실행 → 사용자가 속한 스페이스 목록 조회
2. 스페이스 목록을 요약하여 사용자에게 제시: "이런 스페이스들이 있어요: [목록]. 어떤 스페이스를 활성화할까요?"
3. 사용자가 선택하면 (또는 "알아서 해줘" → 프로필 기반 최적 스페이스 자동 선택):
   - `gobi --json space list-threads --space-slug <slug> --limit 1`로 접근 가능 여부 확인
   - `.gobi/settings.yaml`에 `spaceSlug: <slug>` 직접 설정
4. 에이전트: "[스페이스 이름]에 연결했어요!"

#### 사용자 응답 시나리오

##### A. 스페이스 선택 완료
- 사용자: (스페이스 선택)
- 에이전트: "좋아요! [스페이스 이름]에 참여했어요. 이제 이 스페이스에서 다른 전문가들과 소통할 수 있어요."

##### B. 잘 모르겠는 사용자 / 에이전트에게 위임
- 사용자: "어떤 스페이스가 좋을까요?" / "잘 모르겠어요" / "알아서 해줘"
- 에이전트: 프로필(BRAIN.md)의 관심사를 분석하여 가장 적합한 스페이스를 자동 선택
- 에이전트: "프로필을 보니 [관심 분야]에 관심이 있으시네요. [추천 스페이스]가 딱 맞을 것 같아요. 연결했어요!"

##### C. 나중에/건너뛰기
- 사용자: "나중에요" / "건너뛸게요"
- 에이전트: "알겠어요. 나중에 '스페이스 참여'라고 말하면 언제든 도와드릴게요. 다음으로 넘어갈게요."

### 5-2. 브레인 업데이트 게시 (Post Brain Update / PBU)

> 온보딩 하면서 프로필도 만들고 글도 써봤잖아요. 이걸 바탕으로 첫 브레인 업데이트를 게시해볼까요?
> 브레인 업데이트는 내 브레인 소식을 커뮤니티에 공유하는 거예요. 짧은 인사나 관심사 소개면 충분해요.

**PBU 프롬프트(`_Settings_/Prompts/Post Brain Update (PBU).md`)를 실행**하여 온보딩 중 만든 콘텐츠(프로필, writeup 등)를 소스로 드래프트를 작성한다.
- 소스: 온보딩에서 생성한 BRAIN.md, writeup 등
- 출력: `_Outbox_/BrainUpdates/` 에 드래프트 파일 생성
- **참조 파일 동기화**: 드래프트에서 링크하는 파일(이미지, 관련 분석 등)을 `.gobi/syncfiles`에 추가하여 함께 업로드되도록 한다
- **미디어 파일 포함**: 영상/이미지 등 미디어 파일이 있으면 `.gobi/syncfiles`에 추가하고, `gobi brain edit-update`로 본문에 미디어 임베드(`![](파일경로)`)를 포함한다. 사용자가 "영상도 올려줘" 등 요청하면 즉시 반영한다.
- Obsidian에서 드래프트를 열어 사용자에게 확인 요청

> 이런 내용으로 브레인 업데이트 초안을 만들었어요.
> Obsidian에서 확인해보시고, 수정할 부분이 있으면 말씀해주세요.
> 좋으면 게시할까요?

#### 사용자 응답 시나리오

##### A. 게시 동의
- 사용자: "좋아요, 게시해주세요" / "이대로 할게요"
- 드래프트의 `approve_for_publish`를 `true`로 변경 후 `gobi brain post-update --title "제목" --content "본문"` 실행
- 에이전트: "첫 브레인 업데이트가 게시됐어요! 스페이스에서 다른 사람들이 볼 수 있어요."

##### B. 내용 수정 요청
- 사용자: "내용을 좀 바꾸고 싶어요" / "[수정 내용]으로 바꿔주세요"
- 에이전트: (드래프트 수정 반영 후) "이렇게 수정했어요. 게시할까요?"
- 사용자 확인 후 `gobi brain post-update --title "수정된 제목" --content "수정된 내용"` 실행

##### C. 나중에/건너뛰기
- 사용자: "나중에 할게요" / "건너뛸게요"
- 에이전트: "알겠어요. 나중에 'PBU' 또는 '브레인 업데이트'라고 말하면 언제든 도와드릴게요. 다음으로 넘어갈게요."

#### 쓰레드 연동 제안

브레인 업데이트 게시 직후, 같은 내용을 스페이스 쓰레드에도 공유할지 제안한다.

> 브레인 업데이트가 게시됐어요! 같은 내용으로 스페이스 쓰레드에도 올려볼까요? 더 많은 사람들이 볼 수 있어요.

- 사용자 동의 시: 브레인 업데이트 내용을 기반으로 `gobi space create-thread`로 쓰레드 생성
- 사용자 거절 시: "알겠어요. 나중에 스페이스에서 언제든 올릴 수 있어요."

### 5-3. Welcome 쓰레드에 인사하기

> 스페이스에서 첫 인사를 남겨볼까요?
> Welcome 쓰레드가 있으면 거기에 댓글을, 없으면 새로 인사 글을 올려볼게요.

1. `gobi --json space list-threads --limit 10` 실행 → Welcome/인사 쓰레드 탐색
2. **Welcome 쓰레드가 있는 경우**: 쓰레드 내용을 보여주고 인사 댓글 초안 작성
3. **Welcome 쓰레드가 없는 경우**: 인사 쓰레드 초안 작성 (제목 + 본문)
4. 사용자 프로필(BRAIN.md) 바탕으로 간단한 자기소개 메시지 생성
5. **미디어 포함**: 온보딩 중 만든 영상/이미지가 있으면 `.gobi/syncfiles`에 추가 후 쓰레드 본문에 미디어 임베드(`![](파일경로)`)를 포함한다
6. 사용자 확인 후 게시
...
##### A. 게시 동의
- 사용자: "좋아요, 올려주세요"
- Welcome 쓰레드 있을 때: `gobi space create-reply <threadId> --content "인사 메시지"` 실행
- Welcome 쓰레드 없을 때: `gobi space create-thread --title "인사합니다 👋" --content "인사 메시지"` 실행
- 에이전트: "첫 인사를 남겼어요! 스페이스에서 다른 분들도 볼 수 있어요."

##### B. 내용 수정 요청
- 사용자: "좀 바꾸고 싶어요" / "[수정 내용]"
- 에이전트: (수정 반영 후) "이렇게 수정했어요. 올릴까요?"

##### C. 건너뛰기
- 사용자: "나중에요" / "안 할래요"
- 에이전트: "알겠어요. 나중에 스페이스에서 언제든 대화할 수 있어요."

### 완료 확인 & 전환

> 다섯 번째 단계 완료! 커뮤니티 스페이스에 참여하고, 브레인 업데이트도 게시하고, 첫 인사도 남겼어요.
> 마지막으로 온보딩을 마무리할게요.
...
```
✅ 1단계: PKM과 GD 소개
✅ 2단계: 사용자 파악 — 프로필 완료, 동기화 완료
🔄 3단계: 지식 구축 — 클리핑 ✅ / DRB ✅ / TIU, 캔버스 남음
⬜ 4단계: 콘텐츠 생성
⬜ 5단계: 지식 공유 (선택)
⬜ 6단계: 마무리
```
...
| 이미지 | 표시 시점 | 용도 | 표시 조건 |
|--------|----------|------|----------|
| `graph-example.png` | 3-1 첫 클리핑 & 토픽 생성 시 | 지식 그래프 예시로 클리핑+토픽 연결을 시각적으로 시연 | 3단계를 건너뛰지 않은 경우만 |
...
| `Gobi Onboarding Flow.canvas` | 2단계 오프닝 (캔버스 열기) | 온보딩 상황 캔버스 (Ingest → Summary → Topics → Publish 흐름) | 옵시디언 활성화 시만 |
...
| Space | 주제별 커뮤니티 공간 | 스페이스 |
| Thread | 스페이스 내 게시글/대화 | 쓰레드 |
| Brain Update | 내 브레인 소식 게시 | 브레인 업데이트 |
...
| v3.1 | 2026-03-04 | 5단계 지식 공유: Gobi Space 온보딩 (warp, brain post-update, list-threads) |
| v3.2 | 2026-03-05 | 옵시디언 볼트 열기: 사용자가 옵시디언에서 직접 열도록 안내 (GD와 나란히 사용) |
| v3.3 | 2026-03-05 | 옵시디언 연동 규칙 추가: 파일 자동 열기, 그래프 뷰 갱신, 약속 실행 강제, 볼트 자동 열기 |
| v3.4 | 2026-03-06 | 언어 설정 개선: primaryLanguage 미설정 시 오프닝에서 선호 언어 확인 후 settings.yaml에 저장, 기본값 English |
| v3.5 | 2026-03-06 | 옵시디언 연동을 1단계→2단계로 이동 (1단계 건너뛰기 대응), 언어 불일치 시 선호도 확인 규칙 추가, obsidian CLI 전제 조건 추가 (볼트 열림 확인 전까지 open 시도 금지) |
